...
-- Selecionar empregado e atribuir nomes específicos para as colunas que serão exibidas
SELECT NomeEmpregado as Nome,
		salario as SalarioMensal,
		(salario*12) as SalarioAnual,
		(salario*12) -- "No column name" por que não foi atribuído um nome a ela
FROM Empregado;

-- Selecionar nomes dos empregados e ordenar de maneira descendente
SELECT NomeEmpregado FROM Empregado ORDER BY NomeEmpregado DESC;

-- Padrão: ORDER BY ASCendente.
SELECT NomeEmpregado FROM Empregado ORDER BY NomeEmpregado;

-- Ordenar com base em mais de um valor
SELECT NomeEmpregado, salario FROM Empregado ORDER BY salario DESC, NomeEmpregado ASC;

-- Ordenador utilizando o nome adotado pela coluna
SELECT NomeEmpregado as Nome,
		salario as SalarioMensal,
		(salario*12) as SalarioAnual
FROM Empregado ORDER BY Nome ASC, SalarioMensal DESC;